...
void setup()
{
	lcd_init();
     Sound_Init();
  lcd_print_at_PGM(0, 0, PSTR(" Holy Moly its a :  "));
	lcd_print_at_PGM(0, 1, PSTR("   Custom Hemera    "));
	lcd_print_at_PGM(0, 2, PSTR("  Prusa 3D Printer  "));
  lcd_print_at_PGM(0, 3, PSTR("     Lets Print     "));
	setup_killpin();
	setup_powerhold();
...
if((eSoundMode==e_SOUND_MODE_LOUD)||(eSoundMode==e_SOUND_MODE_ONCE))
  ...
  lcd_print_at_PGM(0, 0, PSTR(" Holy Moly its a :  ")); // we need to do this again for some reason, no time to research
  lcd_print_at_PGM(0, 1, PSTR("   Custom Hemera    "));
  lcd_print_at_PGM(0, 2, PSTR("  Prusa 3D Printer  "));
  lcd_print_at_PGM(0, 3, PSTR("     Lets Print     "));
	card.initsd();
